...
class HillClimbingAlgorithm:
    ...
    def run(self, iterations=100_000):
        ...
        for _ in range(iterations):
            ...
            new_solution_distance = self.calculate_total_distance(new_solution)

            if new_solution_distance < current_total_distance:
                current_solution = new_solution
                current_total_distance = new_solution_distance

        answer_solution = []
        for i in range(len(current_solution) - 1):
            path = self.path_between_cities(current_solution[i], current_solution[i + 1])
            answer_solution.extend(path)
            answer_solution.pop()

        return answer_solution, current_total_distance
...
if __name__ == "__main__":
    ...
    solver = HillClimbingAlgorithm(graph, cities)
    print(solver.run())
